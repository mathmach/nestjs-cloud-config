import { ConfigClientOptions, ConfigObject } from '@nestjs-ext/cloud-config/spring';
...
describe('SpringCloudConfigGatewayImpl', function () {
  ...
  describe('#getConfigFromServer()', function () {
    ...
    it('should throw error if cloud config client throws error', async function () {
      ...
      const getConfigFromServer: Promise<ConfigObject> = springCloudConfigGatewayImpl.getConfigFromServer(defaultOptions);
